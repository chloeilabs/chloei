...
`src/lib/server/integration-flags.ts` resolves default-off flags: `telemetryRecordIo` (`AGENT_TELEMETRY_RECORD_IO`), `responseCompaction` (`AGENT_RESPONSE_COMPACTION` — enables OpenAI server-side context compaction within a run via `modelSettings.contextManagement`; `resolveContextManagementSettings` in `agent-runtime.ts`, threshold `RESPONSE_COMPACTION_TOKEN_THRESHOLD`), `responsesWebsocketTransport` (`AGENT_RESPONSES_WS_TRANSPORT` — routes Responses API traffic over a persistent WebSocket via `configureResponsesTransport` in `openai-client.ts` instead of HTTP), and the three Goblins v2 flags: `goblinsAdaptive` (`AGENT_GOBLINS_ADAPTIVE` — triage tiers, multi-round delegation, coverage evaluator, shared research state; flag-off keeps byte-identical legacy prompts), `goblinsHostedTools` (`AGENT_GOBLINS_HOSTED_TOOLS` — per-role hosted `web_search`/`file_search`/`code_interpreter` on the specialists + per-request vector stores over PDF attachments), and `goblinsBackgroundEscalation` (`AGENT_GOBLINS_BACKGROUND_ESCALATION` — durable background deep-research runs; see Background section). Precedence:
...
- **Webhook receiver** — `POST /api/webhooks/openai` (`src/app/api/webhooks/openai/route.ts`) verifies the Standard-Webhooks signature via `client.webhooks.unwrap` (secret `OPENAI_WEBHOOK_SECRET`; unset → 503; bad signature → 400), de-dupes by webhook id, acks 2xx immediately, and (for `response.*` events whose id maps to a `goblins_run`) drives the background continuation via `after()`. The signing secret is the auth — the route is intentionally public (the middleware matcher doesn't cover `/api/*`). Register the endpoint + secret in the OpenAI dashboard.
- **Background + resumable streaming** — `src/lib/server/llm/background-responses.ts`: `createBackgroundResponse` (`background: true, store: true`, optionally created streaming so clients can live-tail later) returns an id as soon as it is known; `resumeBackgroundResponseStream` streams a stored response from a `starting_after` checkpoint. `GET /api/agent/responses/[responseId]/stream?after=<seq>` resumes as NDJSON (ownership-checked via the goblins run that owns the response id).
- **Goblins background escalation** (`agent.goblins.background_escalation`, default off) — durable deep-research runs. Each GPT-5.5 manager turn runs as an OpenAI background response (chained via `previous_response_id`); the `response.completed` webhook (or a stale-run client poll) wakes `continueGoblinsRun` (`src/lib/server/llm/goblins-background-run.ts`), which executes the turn's local tool calls (goblin sub-runs with Exa, `coverage_check`) inside that invocation, persists to the `goblins_run` table (`src/lib/server/goblins-run-store.ts`: lease-based claim = cross-instance dedupe; `toolResults` make re-drives idempotent; append-only event log for client replay), and dispatches the next turn. The final turn folds the answer into the thread. Escalation trigger: `background: true` on `/api/agent` (goblins model + flag + `threadId`; UI: "Deep research" toggle in the prompt form). Client follows via `GET /api/agent/goblins-runs/[runId]?afterEvent=<n>` polls (+ cancel via `POST .../cancel`); reload-resume replays the event log through the normal accumulator. No cron: wake sources are user requests, webhook deliveries (72h retries), and client polls only; abandoned runs expire lazily (~3h TTL).
